-- === Migration 1: 20260720070000_rfq_quotation_foundation.sql ===
...
create policy "rfq_quotation_participant_read"
  ...
  using (
    ...
    or exists (
      select 1 from public.companies c
      where c.id = recipient_company_id and c.owner_id = auth.uid()
    )
    or public.has_role(_role => 'admin'::public.app_role, _user_id => auth.uid())
  );
...
create policy "rfq_quotation_verified_sender_insert"
  ...
  with check (
    ...
    and exists (
      select 1 from public.rfq_listings r
      where r.id = rfq_id
...
-- === Migration 2: 20260720070200_replace_legacy_rfq_policies.sql ===
do $$
declare
...
begin
  for existing_policy in
    ...
    where schemaname = 'public' and tablename = 'rfq_listings'
  loop
    execute format('drop policy if exists %I on public.rfq_listings', existing_policy.policyname);
  end loop;
end $$;
...
-- === Migration 3: 20260720070300_quotation_validity_guard.sql ===
drop policy if exists "rfq_quotation_verified_sender_insert" on public.rfq_quotations;
create policy "rfq_quotation_verified_sender_insert"
  on public.rfq_quotations
  for insert
