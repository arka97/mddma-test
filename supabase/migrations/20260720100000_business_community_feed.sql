...
--
-- Product boundary:
-- - Feed content is publicly readable.
-- - Only approved, visible, verified businesses may post, comment, like, or vote.
-- - Public attribution is to the business, while author_id remains the accountable user.
-- - Anonymous posting is retired; historical anonymous posts are hidden.
-- - Sourcing requirements belong in the RFQ workflow, not new feed posts.
...
-- Replace legacy paid/trial/anonymous policies with one explicit business policy set.
do $$
declare
  policy_row record;
begin
...
create policy post_comments_owner_or_admin_delete
  on public.post_comments
  for delete
  to authenticated
  using (
...
create policy anonymous_identity_log_admin_read
  on public.anonymous_identity_log
  for select
  to authenticated
  using (public.has_role(_role => 'admin'::public.app_role, _user_id => auth.uid()));
...
declare
  new_post_id uuid;
  normalised_topic text;
begin
  if not public.has_verified_business() then
    raise exception 'An approved, visible, verified business is required';
  end if;

  if _post_type not in ('general', 'price_signal', 'market_alert', 'member_news') then
    raise exception 'Unsupported community post type';
  end if;

  if char_length(btrim(coalesce(_content, ''))) > 5000 then
    raise exception 'Post content cannot exceed 5000 characters';
  end if;

  if btrim(coalesce(_content, '')) = '' and _structured_data is null then
...
create or replace function public.create_business_poll_post(
  _question text,
  _options text[],
  _duration_days integer default 3,
  _content text default '',
  _structured_data jsonb default null
)
returns uuid
...
begin
  ...
  if char_length(btrim(coalesce(_question, ''))) not between 3 and 240 then
    raise exception 'Poll question must contain between 3 and 240 characters';
  end if;

  select count(*), count(distinct lower(btrim(option_value)))
    into option_count, distinct_option_count
  from unnest(coalesce(_options, array[]::text[])) as option_value
  where btrim(option_value) <> '';

  if option_count not between 2 and 4 or distinct_option_count <> option_count then
    raise exception 'Polls require 2 to 4 unique options';
  end if;

  if _duration_days not between 1 and 14 then
    raise exception 'Poll duration must be between 1 and 14 days';
  end if;
...
create or replace function public.add_business_comment(
  _post_id uuid,
  _content text
)
returns uuid
...
begin
  ...
  if char_length(btrim(coalesce(_content, ''))) not between 1 and 1500 then
    raise exception 'Comment must contain between 1 and 1500 characters';
  end if;
...
create or replace function public.set_business_post_like(
  _post_id uuid,
  _liked boolean
)
returns boolean
...
create or replace function public.cast_business_poll_vote(
  _poll_id uuid,
  _option_id uuid
)
returns uuid
...
begin
  ...
  if not exists (
    ...
    where poll.id = _poll_id
      and option_row.id = _option_id
  ...
  where poll_id = _poll_id and voter_id = auth.uid();

  insert into public.post_poll_votes (poll_id, option_id, voter_id)
  values (_poll_id, _option_id, auth.uid());
...
as $$
  ...
  group by poll.id, poll.post_id, poll.question, poll.closes_at,
...
as $$
  select
    ...
    auth.uid() is not null and exists (
      ...
      where mine.post_id = post.id and mine.user_id = auth.uid()
    )
  from public.community_posts post
...
-- Community media is publicly readable because the feed is public. Upload/delete
-- stays restricted to an approved verified business and that user's own folder.
