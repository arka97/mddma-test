-- Membership-aware helpers: allow any active company_members role to act on behalf
-- of an approved, visible, verified business (not just the original owner_id).
...
-- get_my_company: pick owned first, then any membership-linked verified company.
...
-- start_deal_room: allow any owner/admin/editor member to initiate on behalf of company.
...
-- send_deal_message: accept membership-linked sender.
...
-- can_access_deal_room: allow members of either party (not only owner_id).
      ...
      where room.id = _room_id
        and (
          exists (
            ...
            where c.id in (room.initiator_company_id, room.counterparty_company_id)
              and c.owner_id = auth.uid()
          )
          or exists (
            select 1 from public.company_members m
            where m.company_id in (room.initiator_company_id, room.counterparty_company_id)
              and m.user_id = auth.uid()
              and m.role in ('owner','admin','editor')
          )
        )
...
-- RFQ listings: extend insert/update policies to accept membership.
...
create policy "rfq_owner_update"
  ...
  using (
    ...
    or exists (
      select 1 from public.company_members m
      where m.company_id = rfq_listings.company_id
        and m.user_id = auth.uid()
