...
A **Behavioral Trade Operating System** for the Association: a verified member directory, seller storefronts and brand pages, a price-controlled product catalogue, admin-managed circulars + advertising + market-news CMS, a native community archive plus a Discourse-embedded live forum, and a public knowledge layer — wrapped in an installable PWA, role-aware (Guest / Free / Paid / Broker / Admin), DPDP-compliant, and built on Lovable Cloud (Auth + Postgres + Storage + Edge Functions). No exact prices, no exact stock counts are ever rendered — that is the "controlled transparency" rule (UX-001) the rest of the spec is built around. Contact between buyers and sellers happens via `wa.me` deeplinks and revealed phone numbers, not an in-app RFQ engine.
...
| Admin CMS (circulars + ads + market news) | **Live** | doc 04, doc 13 |
| Home modules: Today header, Live ticker, Quick actions, Category grid, **New Products**, **New Members**, Membership CTA, Partners strip | **Live** | doc 04 |
| Forms page (Advertise + Submit Circular tabs) | **Live** | doc 04 |
| KYC verification (admin-managed via `/account/moderation`) | **Live** | doc 23 |
| Native forum (posts + comments) | **Live** (read-only archive; Discourse embed is the primary forum) | doc 04 |
...
| RFQ engine, multi-item cart, `/account/rfqs` inbox | **Removed v3.1.3** — never returning | doc 11 |
...
| Transactional core (directory, products, storefronts, broker, dashboard) | **Live** — `noindex` + auth-walled (GTM-001) | doc 11 |
