...
export function CategoryGrid({ heading = "Browse categories", subtitle = "Pick a category to explore listings" }: { heading?: string; subtitle?: string }) {
  ...
  const renderCard = (cat: typeof cats[number]) => {
    ...
    return (
      ...
      >
        {hot && (
          <span className="absolute left-1.5 top-1.5 z-10 inline-flex items-center gap-0.5 rounded bg-accent px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-gold-dark-foreground shadow">
